...
public class Drive extends SubsystemBase {
    ...
    // MetersPerSecond.of(0.05); // Need to be under this to switch to coast when
    // disabling
    // private static final Measure<Velocity<Distance>> coastThresholdSecs =
    // MetersPerSecond.of(6.0); // Need to be under the above speed for this length
    // of time to switch to coast
    ...
    // private PIDController linearController = new PIDController(kPHeading,
    // kIHeading, coastThresholdMetersPerSec);
    ...
    // reallocation.
    // private final MutableMeasure<Distance> characterizationDistance =
    // mutable(Meters.of(0));
    // // Mutable holder for unit-safe linear velocity values, persisted to avoid
    // // reallocation.
    // private final MutableMeasure<Velocity<Distance>> characterizationVelocity =
    // mutable(MetersPerSecond.of(0));

    // Mutable holder for unit-safe angular distance values, persisted to avoid
    // reallocation.
    ...
    public void periodic() {
        ...
        // TODO: make sure there's no harm in turning this into a method (it was just
        // written here before)
        ...
        switch (controlMode) {
            ...
            case WII_SETPOINT:
                ...
                rotVelocity = MathUtil.clamp(
                        rotVelocity,
                        getVelocity().dtheta
                                - getMaxAngularAcceleration().in(RadiansPerSecond.per(Second)) * Constants.PERIOD,
                        getVelocity().dtheta
                                + getMaxAngularAcceleration().in(RadiansPerSecond.per(Second)) * Constants.PERIOD);
            ...
            case CHASSIS_SETPOINT:
                ...
                // double[] accelVector = {
                // (adjustedSpeeds.vxMetersPerSecond - lastSetpoint.vxMetersPerSecond) /
                // Constants.PERIOD,
                // (adjustedSpeeds.vyMetersPerSecond - lastSetpoint.vyMetersPerSecond) /
                // Constants.PERIOD
                // };
                ...
                // adjustedSpeeds.vxMetersPerSecond = lastSetpoint.vxMetersPerSecond +
                // Math.cos(accelDir) * maxLinearAcceleration * Constants.PERIOD;
                // adjustedSpeeds.vyMetersPerSecond = lastSetpoint.vyMetersPerSecond +
                // Math.sin(accelDir) * maxLinearAcceleration * Constants.PERIOD;
                // }
